...
**One shared SQLite DB for all runs and all worktrees**: `config/test.exs` pins `:data_dir` to `System.tmp_dir!()/evogit_test_data/genesis` and `System.tmp_dir!()` reads `TMPDIR`, so two concurrent `mix test` runs (or a run polluted by a sibling app's leftover rows) can cross-contaminate whole-table assertions; re-run the suspect file in isolation before treating a failure as a regression, or run with a private `TMPDIR=$(mktemp -d)`.
