...
- `task_export_controller_test.exs` is `async: true`: it writes only uniquely-id'd `export_test_*` rows (unconditional `on_exit` cleanup), and its 3 `?node=` tests isolate `XDG_CONFIG_HOME` (save/restore in `on_exit`). It is the sole `async: true` module that SETS `XDG_CONFIG_HOME` — every other setter is `async: false` — so ExUnit's async/sync phase separation keeps them from overlapping. The other async ConnCase users here (`error_html_test` / `error_json_test`) are pure render tests.
