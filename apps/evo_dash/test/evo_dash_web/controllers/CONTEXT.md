...
- `task_export_controller_test.exs` does NOT isolate the Store/TaskRegistry (lines 174-196): `seed_completed_task/1` writes into the PRODUCTION `EvoGit.Store` (line 191, unique `export_test_<int>` ids) and `on_exit` (line 193) deletes the row via `TaskRegistry.delete_task/1` + a `TaskRegistry.list_tasks()` cast-sync (`cleanup_task/1`, lines 198-202); the comment's rationale is that a leaked row breaks `evo_git`'s task-list tests, because both umbrella apps in one `mix test` run share the run's single unique data dir (`config/test.exs`'s `:evo_git, :data_dir` = `System.tmp_dir!()/evogit_test_data/genesis-<pid>-<int>`).
- The three tests in `describe "...with ?node= param"` replace the process-wide `XDG_CONFIG_HOME` with a unique temp dir (lines 106-129) so `EvoGit.RemoteConnections` (a pure-function TOML module reading `EvoGit.Config.config_dir/0`) never touches the developer's real `~/.config/genesis`; restored in `on_exit`.
